...
Sub Draft_Email()
    ...
    ' --- Build groups: filter by Col P = "FA" AND Col AS = "A" ---
    ...
    ' --- Output folder (🔧 replace this later) ---
    outputFolder = "C:\YOUR\OUTPUT\FOLDER\"   ' <-- change to your real path
    ...
    ' --- Loop through each group (each unique B) ---
    For Each dictKey In dict.Keys
        ...
        ' Write headers (F, N, J, M, T in that order)
        targetRow = 1
        ...
        If targetRow > 1 Then   ' only sort if there is data
            With wsNew.Sort
                .SortFields.Clear
        ...
        ' Use name in Column M for filename (from first row in the group)
